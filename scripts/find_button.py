...
def find_trash_button(port=9222):
    ...
    with sync_playwright() as p:
        try:
            ...
            # Wylistujmy wszystkie elementy, które mają 'usuń' lub 'remove'
            print("[*] Klikam 'Usuń opcje' (strzałeczkę obok kosza)...")
            options_btn = target_page.locator('button[aria-label="Usuń opcje"]')
            if not options_btn.count():
                options_btn = target_page.locator('button[aria-label="Remove options"]')
            ...
            if options_btn.count():
                ...
                print("Nie znaleziono przycisku opcji kosza.")
                
            print("[*] Szukam 'Usuń wskaźniki' w menu dolnym...")
            menu_items = target_page.locator('[data-role="menuitem"]').all()
            for el in menu_items:
                text = el.inner_text().strip()
                if re.search(r"Usuń wskaźnik|Remove indicator", text, re.IGNORECASE):
                    print(f"[+] Znalazłem i klikam: '{text}'")
                    el.click(force=True)
                    time.sleep(1)
                    break
