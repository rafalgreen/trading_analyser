...
- **Trendy + PCA** — 2× Wzrostowy + PCA ≤ 40 → Strong Buy; 2× Wzrostowy → Buy; 2× Spadkowy + PCA ≥ 60 → Strong Sell; 2× Spadkowy → Sell; reszta → Neutral. PCA niskie = okazja (buy), wysokie = drogo (sell) — zgodnie z koszami PCA.
- **Crossy (priorytet)** — BULL/BEAR CROSS z HTS/MacD przeważają nad trendem; PCA jako tie-breaker (≤40 buy, ≥60 sell). Fallback na trend jeśli brak crossów.
- **PCA (kosze)** — tylko PCA: ≤20 Strong Buy, 20–40 Buy, 40–60 Neutral, 60–80 Sell, ≥80 Strong Sell. **Semantyka PCA (risk-on): niskie wartości = okazja/bullish, wysokie = drogo/bearish** — spójne we wszystkich strategiach używających PCA.
